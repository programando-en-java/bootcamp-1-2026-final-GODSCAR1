...
final class FlightSpecifications {
    ...
    static Specification<FlightEntity> departingAfter(final Instant instant) {
        return (root, query, builder) ->
                builder.greaterThan(root.get(FlightEntity_.departureTime), instant);
    }

    static Specification<FlightEntity> departingBefore(final Instant instant) {
        return (root, query, builder) ->
                builder.lessThan(root.get(FlightEntity_.departureTime), instant);
    }

    static Specification<FlightEntity> withOrigin(final AirportCode origin) {
        return (root, query, builder) ->
                builder.equal(root.get(FlightEntity_.origin), origin.value());
    }

    static Specification<FlightEntity> withDestination(final AirportCode destination) {
        return (root, query, builder) ->
